...
describe("POST /api/webhooks/waha", () => {
  beforeEach(() => {
    ...
    mocks.supabaseRpcMock.mockResolvedValue({
  ...
  it("prioritizes pending post-stay reservation over older completed handoff reservation for the same guest", async () => {
    ...
    expect(mocks.processLifecycleGuestMessageMock).not.toHaveBeenCalled();
    expect(mocks.supabaseRpcMock).toHaveBeenCalledWith(
      "complete_post_stay_feedback_with_reward",
      expect.objectContaining({
        p_reservation_id: "reservation-pending-new",
        p_tenant_id: "tenant-1",
        p_rating: 5,
        p_comments: "Rating saya 5, terima kasih",
      }),
    );
