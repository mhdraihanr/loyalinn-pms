...
- **Realtime WhatsApp inbox** — database-backed floating drawer with Supabase Realtime, protected server-side WAHA sends, optimistic staff replies, and one canonical direct-chat thread per tenant/session/normalized phone even when WAHA reports `@c.us` and `@lid` aliases.
...
- The WhatsApp inbox persists direct text conversations in Supabase and subscribes through Supabase Realtime, so new inbound, automated outbound, and staff outbound messages appear without a browser refresh. Transcript loading is request-safe when staff switch chats, and each provider/client message identity is reconciled into one durable `whatsapp_messages` row before Realtime renders it. The **Sync now** control is only a recovery/reconciliation action.
- Apply `supabase/migrations/20260804000000_add_realtime_whatsapp_inbox.sql`, then `supabase/migrations/20260804000001_add_whatsapp_conversation_identity.sql`, then `supabase/migrations/20260804000002_add_whatsapp_canonical_conversation_key.sql`, and finally `supabase/migrations/20260804000003_add_whatsapp_message_idempotency.sql` manually before enabling the realtime inbox. The third migration consolidates existing `@c.us`/`@lid` aliases into one canonical direct-contact thread; the fourth adds provider/client idempotency keys so WAHA `fromMe` echoes reconcile with existing manual or AI outbound rows instead of creating a second bubble. Apply the consolidation migrations during a short webhook write pause.
- Inbox direct threads are keyed by tenant, WAHA session, and resolved normalized phone—not reservation and not raw WAHA chat ID. The app keeps the newest valid provider alias only as the protected WAHA send target. Existing conversations with an unambiguous known guest phone self-heal when loaded; ambiguous numbers deliberately remain number-only.
- The current WAHA Core deployment is intentionally limited to session `default`; set `WAHA_DEFAULT_TENANT_ID` to the one tenant that owns it. Other tenants cannot access this WAHA inbox. Use a per-tenant session mapping before operating WAHA inboxes for multiple tenants.
