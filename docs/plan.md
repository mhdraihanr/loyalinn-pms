...
- Webhook-first PMS runtime, with pull sync reserved for reconciliation and recovery
- For QloApps, reservation lifecycle truth must come from room booking status after enrichment; order/payment status alone must not be treated as `on-stay`
- Lifecycle outbound messages are once-per-reservation-per-trigger after a successful send; `on-stay` realtime jobs require an actual transition into `on-stay`, not any unrelated reservation update while the reservation is already `on-stay`
...
- Do not rely on default browser interval refresh for PMS correctness; webhook ingestion is the source of truth and browser push/manual refresh can be layered separately.
- For QloApps webhook-first runtime, keep order/payment events and room-status events as separate domains; room-status transitions are required to drive `on-stay` and `checked-out` behavior.
- For duplicate prevention, do not use single-row assumptions when checking sent lifecycle logs. Historical duplicate `message_logs` rows must still be treated as proof that a trigger was already delivered.
...
- Automation dev scheduler: DEV_AUTOMATION_SYNC_ENABLED, DEV_AUTOMATION_SYNC_INTERVAL_MS
...
For QloApps, the production runtime should operate in these layers:

1. **Webhook-first mode**
   - New bookings and status changes enter through `POST /api/webhooks/pms`.
   - Event payloads are verified, normalized, deduplicated, enriched, and persisted before automation is enqueued.
2. **Fallback reconciliation mode**
   - `GET /api/cron/pms-sync` remains available for low-frequency recovery and audit use.
   - Reconciliation must not be treated as the primary change feed.
3. **Development-only pull mode**
   - Interval pull sync can be enabled temporarily for adapter debugging, but it is opt-in.

### Operational note

If operators still observe periodic development logs after webhook-first cutover, check the development automation scheduler before concluding that PMS sync is still polling.
...
- ✅ WAHA post-stay direct-rating replies now use a deterministic parser before AI dispatch, so explicit messages like `Rate dari saya 5 kamar luas` complete feedback and reward points without relying on probabilistic tool calling.
...
- ✅ Operations and Feedback dashboard tables now match Reservations/Guests interaction patterns with table headers, client-side search, full empty states, and no-result search states.
...
- ✅ Feedback Monitor table now follows the Reservations/Guests table pattern with header helper text, client-side search, source-empty state, and no-result search state.
...
- **Post-stay feedback loop (Hybrid Web-Form + AI Agent):**
  ...
  6. **Deterministic Direct Rating:** If the inbound WhatsApp reply already contains an explicit 1-5 rating (for example, `Rate dari saya 5 kamar luas`), the WAHA webhook saves `post_stay_rating`, `post_stay_comments`, completes the feedback, and applies reward points before calling the AI agent.
  7. **Function Calling & Summarization:** For ambiguous replies without an explicit rating, AI continues the conversation, asks for a numeric 1-5 rating when needed, summarizes the guest's unstructured chat reply, and calls `update_guest_feedback(rating, comments)` to save structured data back to the database, identical to the Web Form output.
